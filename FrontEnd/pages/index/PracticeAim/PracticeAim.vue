...
<!-- 				  <button 
			  ...
			  <div class="mt-4 pt-4 border-t border-gray-100">
				...
				<div v-if="isReviewMode" class="mt-2">
				  ...
				  >
					{{ language === 'en' ? 'Review' : '检查模式'}} {{ showReviewDropdowns[index] ? '▲' : '▼' }}
				  </button>
...
<!-- 					  <div>Accuracy: <span class="font-medium">{{ reviewScores[index]?.text_score || 0 }}%</span></div>
					  ...
					  <div>{{ language === 'en' ? 'Text Score' : '清晰得分'}}: <span class="font-medium">{{ textScores[index] }}%</span></div>
					  <div>{{ language === 'en' ? 'Duration Score' : '语速得分'}}: <span class="font-medium">{{ durationScores[index] }}%</span></div>
					  <div>{{ language === 'en' ? 'Pause Score' : '停顿得分'}}: <span class="font-medium">{{ pauseScores[index] }}%</span></div>
					  <div>{{ language === 'en' ? 'Tempo Score' : '节奏得分'}}: <span class="font-medium">{{ tempoScores[index] }}%</span></div>
					  <div>{{ language === 'en' ? 'Pitch Score' : '音高得分'}}: <span class="font-medium">{{ pitchScores[index] }}%</span></div>
					</div>
		...
		<div v-if="popupVisible" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
		  <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 relative">
		    ...
		    <h2 class="text-2xl font-bold text-center mb-4 text-indigo-800">
		      {{ language === 'en' ? 'Practice Results:' : '练习结果:'}} <span class="text-indigo-600">{{ finalScore }}%</span>
		    </h2>
		    
			<div  class="mt-4 p-4 bg-indigo-50 rounded-lg">
				<div class="grid grid-cols-2 gap-2 text-sm">
					<div v-if="language === 'en'">
						<div>Accumulation Text Score&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: <span class="font-medium">{{ textScore }}%</span></div>
						<div>Accumulation Duration Score: <span class="font-medium">{{ durationScore }}%</span></div>
						<div>Accumulation Pause Score&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: <span class="font-medium">{{ pauseScore }}%</span></div>
						<div>Accumulation Tempo Score&nbsp;&nbsp;&nbsp;: <span class="font-medium">{{ tempoScore }}%</span></div>
						<div>Accumulation Pitch Score&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: <span class="font-medium">{{ pitchScore }}%</span></div>
				    </div>		
 		   			<div v-else>
						<div>最终朗读清晰得分: <span class="font-medium">{{ textScore }}%</span></div>
						<div>最终朗读速度得分: <span class="font-medium">{{ durationScore }}%</span></div>
						<div>最终停顿次数得分: <span class="font-medium">{{ pauseScore }}%</span></div>
						<div>最终朗读节奏得分: <span class="font-medium">{{ tempoScore }}%</span></div>
						<div>最终朗读音调得分: <span class="font-medium">{{ pitchScore }}%</span></div>				  
					</div>
					</div>
			</div>
			
			<div
			  v-if="finalFeedback !== null" 
			  class="mt-4 p-4 bg-indigo-50 rounded-lg"
			>
			  <h3 class="font-semibold text-indigo-800 mb-2">{{ language === 'en' ? 'Feedback:' : '反馈:' }}</h3>
			  <p class="text-gray-700">{{ finalFeedback }}</p>
			</div>
		    ...
		    <div class="flex flex-col space-y-4 mt-6">
		      ...
		      >
				...
				{{ language === 'en' ? 'Review Detail' : '查看评分' }}
		      </button>
		      ...
		      >
				...
				{{ language === 'en' ? 'Try Other Modes' : '其他模式' }}
		        <!-- Try Other Modes -->
		      </button>
		    </div>
		  </div>
...
export default {
  ...
  methods: {
	...
	generateFeedback(index) {
	  // Get scores for this specific line
	  const textScore = this.textScores[index] || 0;
	  const durationScore = this.durationScores[index] || 0;
	  const pauseScore = this.pauseScores[index] || 0;
	  const tempoScore = this.tempoScores[index] || 0;
	  const pitchScore = this.pitchScores[index] || 0;
	  
	  // Text accuracy feedback
	  let textFeedback = '';
	  if (textScore >= 90) {
	    textFeedback = 'Excellent pronunciation with nearly perfect accuracy.';
	  } else if (textScore >= 75) {
	    textFeedback = 'Good pronunciation with minor errors.';
	  } else if (textScore >= 50) {
	    textFeedback = 'Several pronunciation errors - focus on word endings.';
	  } else {
	    textFeedback = 'Many pronunciation errors - practice individual words.';
	  }
	  
	  // Duration feedback
	  let durationFeedback = '';
	  if (durationScore >= 90) {
	    durationFeedback = 'Perfect timing for each word.';
	  } else if (durationScore >= 75) {
	    durationFeedback = 'Good timing with minor inconsistencies.';
	  } else if (durationScore >= 50) {
	    durationFeedback = 'Some words held too long or too short.';
	  } else {
	    durationFeedback = 'Timing needs significant improvement.';
	  }
	  
	  // Pause feedback
	  let pauseFeedback = '';
	  if (pauseScore >= 90) {
	    pauseFeedback = 'Natural pauses at perfect locations.';
	  } else if (pauseScore >= 75) {
	    pauseFeedback = 'Most pauses are well placed.';
	  } else if (pauseScore >= 50) {
	    pauseFeedback = 'Some unnatural pauses between words.';
	  } else {
	    pauseFeedback = 'Pauses are too frequent or misplaced.';
	  }
	  
	  // Tempo feedback
	  let tempoFeedback = '';
	  if (tempoScore >= 90) {
	    tempoFeedback = 'Perfect speaking rate - neither too fast nor slow.';
	  } else if (tempoScore >= 75) {
	    tempoFeedback = 'Good speaking pace with minor variations.';
	  } else if (tempoScore >= 50) {
	    tempoFeedback = tempoScore > 65 ? 
	      'Slightly too fast - try slowing down.' : 
	      'Slightly too slow - try speaking faster.';
	  } else {
	    tempoFeedback = tempoScore > 40 ? 
	      'Much too fast - difficult to understand.' : 
	      'Much too slow - sounds unnatural.';
	  }
	  
	  // Pitch feedback
	  let pitchFeedback = '';
	  if (pitchScore >= 90) {
	    pitchFeedback = 'Perfect pitch variation - very natural.';
	  } else if (pitchScore >= 75) {
	    pitchFeedback = 'Good pitch with natural inflection.';
	  } else if (pitchScore >= 50) {
	    pitchFeedback = 'Some monotone sections - vary pitch more.';
	  } else {
	    pitchFeedback = 'Very flat or unnatural pitch variation.';
	  }
	  
	  // Combine all feedback for this line
	  return `${textFeedback} ${durationFeedback} ${pauseFeedback} ${tempoFeedback} ${pitchFeedback}`;
	},

	
	generateFinalFeedback() {
	  // Calculate average scores
	  const avgText = this.textScore;
	  const avgDuration = this.durationScore;
	  const avgPause = this.pauseScore;
	  const avgTempo = this.tempoScore;
	  const avgPitch = this.pitchScore;
	  
	  // Determine overall performance level
	  const overallScore = (avgText + avgDuration + avgPause + avgTempo + avgPitch) / 5;
	  
	  if (overallScore >= 90) {
	    return "Outstanding performance! Your pronunciation, pacing, and intonation are excellent.";
	  } else if (overallScore >= 75) {
	    return "Good performance overall. Focus on the specific areas highlighted for each line to improve further.";
	  } else if (overallScore >= 50) {
	    return "Fair performance. With practice on the areas highlighted for each line, you can significantly improve.";
	  } else {
	    return "Needs more practice. Focus on one aspect at a time (like pronunciation first) before working on others.";
	  }
	},
  ...
  handleScroll() {
	...
	async sendAudioToASR(audioBlob, index) {
	  ...
	  while (attempts < maxRetries && !success) {
		try {
		  attempts++;
		  console.log(`Attempt ${attempts} for line ${index + 1}`);
		  const formData = new FormData();
		  formData.append('files', audioBlob, `recording_${index}.wav`);
		  ...
		  if (jsonResponse.text) {
			this.$set(this.asrResponseLines, index, jsonResponse.text);
			this.textScores[index] = jsonResponse.text_score || 0;
			this.durationScores[index] = jsonResponse.duration_score || 0;
			this.pauseScores[index] = jsonResponse.pause_score || 0;
			this.tempoScores[index] = jsonResponse.tempo_score || 0;
			this.pitchScores[index] = jsonResponse.pitch_score || 0;
			success = true;
			
			await this.calculateScore(index);
			
			// If successful and not the last line, move to next line
			if (this.currentRecordingIndex < this.sentenceLines.length - 1) {
			  this.currentRecordingIndex++;
			  this.$nextTick(() => {
				this.autoScrollToCurrentLine();
		  ...
		  if (attempts >= maxRetries) {
			this.$set(this.asrResponseLines, index, 'Recognition failed - please try recording again');
			this.currentRecordingIndex = index;
			this.canRecord = true;
			
			// Scroll to the failed line
			this.scrollToLine(index);
			
			uni.showToast({
			  title: 'Please re-record this line',
			  icon: 'none',
			  duration: 3000
			});
		  } else {
	...
	async calculateScore(index) {
	  try {
		// Get the recording for this specific index
		// const recording = this.allRecordings[index];
		const allRecorded = this.allRecordings.every(rec => rec !== null);
		const allResponsesReceived = this.textScores.every(score => score !== null);
		
		console.log(index, this.textScores[index], this.durationScores[index], this.pauseScores[index], this.tempoScores[index], this.pitchScores[index]);
		// console.log(recording);
		// Check if all lines have been recorded
		if (allRecorded && allResponsesReceived) {
	...
	async calculateFinalScore() {
	  ...
	  this.textScore = Math.round(this.textScore / n);
	  this.durationScore = Math.round(this.durationScore / n);
	  this.pauseScore = Math.round(this.pauseScore / n);
	  this.tempoScore = Math.round(this.tempoScore / n);
	  this.pitchScore = Math.round(this.pitchScore / n);
	  
	  this.finalScore = Math.round((this.textScore + this.durationScore + this.pauseScore + this.tempoScore + this.pitchScore) / 5);
	  
	  // Generate final feedback
	  this.finalFeedback = this.generateFinalFeedback();
	  console.log("final Feedback: ", this.finalFeedback);
	  this.showResultsPopup();
	},
  },
};
