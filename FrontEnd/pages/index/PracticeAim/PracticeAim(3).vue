	...
	async calculateScore(index) {
	  try {
		...
		if (!recording || !recording.blob) {
		  console.error('No recording found for index:', index);
		  return;
		}

		// Create form data with the single recording
		const formData = new FormData();
		formData.append('files', recording.blob, `recording_${index}.wav`);
		...
		// Store the scores for this specific line
		...
		const calculateAverage = (arr) => {
		  const validScores = arr.filter(score => score !== null);
		...
		// Store review data for this line
		this.$set(this.reviewScores, index, {
		  text_score: jsonResponse.text_score || 0,
		  fluency_score: jsonResponse.duration_score || 0,
		  tempo_score: jsonResponse.tempo_score || 0,
		  pitch_score: jsonResponse.pitch_score || 0,
		  feedback: jsonResponse.feedback || "No specific feedback available"
		});
		
		// Check if all lines have been recorded
		if (this.allRecordings.every(rec => rec !== null)) {
